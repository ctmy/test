...
class Admin extends HYBBS {
    ...
    public function post(){
        ...
        $Post = S("Post");
        $User = M('User');
        $Show_mun = BBSCONF('admin_show_post');
        $where = [
            "ORDER"=> ['pid'=>'DESC'],
            "LIMIT" => array(($pageid-1) * $Show_mun, $Show_mun)
        ];
        $data = $Post->select('*',$where);
        $count = $Post->count();
        $page_count = ($count % $Show_mun != 0)?(intval($count/$Show_mun)+1) : intval($count/$Show_mun);
    ...
    public function post_post(){
        ...
        $User = M('User');
        $Show_mun = BBSCONF('admin_show_post_post');
        $where = [
            "ORDER"=> ['id'=>'DESC'],
            "LIMIT" => array(($pageid-1) * $Show_mun, $Show_mun)
        ];
        $data = $Post_post->select('*',$where);
        $count = $Post_post->count();
        $page_count = ($count % $Show_mun != 0)?(intval($count/$Show_mun)+1) : intval($count/$Show_mun);
